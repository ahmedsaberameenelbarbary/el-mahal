...
@SessionScoped
@Entity
public class useres1 extends CRUD implements Serializable, share {
    ...
    private List alluseres1() {
        Session s = Util.getSessionFactory().openSession();
        List<useres1> cus = new ArrayList<useres1>();
        try {
            s.beginTransaction();

            org.hibernate.Query q = s.createQuery("from useres1 ");
            cus = q.list();
...
//check account
    ...
    public void checkaccount() {
        ...
        List<useres1> c = customselect(id);
        if (c.isEmpty()) {
...
//check user
    ...
    private String check(String a, String b) {
        ...
        try {
            s.beginTransaction();
            org.hibernate.Query q = s.createQuery("from useres1 where name =? and pass=?");
            q.setParameter(0, a);
            q.setParameter(1, b);
            cus = q.list();
            s.getTransaction().commit();
    ...
    public String count() {
        ...
        try {
            s.beginTransaction();
            org.hibernate.Query q = s.createQuery(" select count(name)from useres1");
...
//buttons function
    ...
    @Transient
    public List<useres1> getTable() {
        return table = alluseres1();
    }

    public void setTable(List<useres1> table) {
        this.table = table;
    }
